...
export class EditTareaComponent implements OnInit, OnDestroy {
  ...
  async crearTarea(): Promise<void> {
    ...
    const nuevaTarea: Partial<Tarea> = {
      ...
      fotoEmpleadoAsignado: empleadoSeleccionado.imagenBase64 ?? '',
    ...
    try {
      ...
      this.alertService.open({ title: 'Tarea Creada', message: 'La tarea se ha creado exitosamente', type: 'success' });

      this.router.navigate(['administracion/gestion-tareas']);
    } catch (error) {
      this.alertService.open({ title: 'Error', message: 'No se pudo crear la tarea', type: 'error' });
    } finally {
      this.isSubmitting.set(false);
    }
  }
  ...
  async guardarCambios(): Promise<void> {
    ...
    if (empleadoSeleccionado) {
      ...
      datosParaActualizar.fotoEmpleadoAsignado = empleadoSeleccionado.imagenBase64 ?? '';
    }
    ...
    try {
      ...
      this.alertService.open({ title: 'Tarea actualizada!!', message: 'La tarea se ha actualizado exitosamente', type: 'success' });

      this.router.navigate(['administracion/gestion-tareas']);
    } catch (error) {
      console.error('Error al actualizar tarea:', error);
      this.alertService.open({ title: 'Error', message: 'No se pudo actualizar la tarea', type: 'error' });
    } finally {
      this.isSubmitting.set(false);
    }
  }
