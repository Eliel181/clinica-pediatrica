  ...
  guardarCambios(): void {
    ...
    this.tareasService.updateTarea(tareaActual.id, datosNuevos)
      .then(async () => {
        ...
        this.alertService.open({ title: 'Tarea actualizada', message: 'Los cambios fueron guardados exitosamente.', type: 'success' });

      }).catch(err => {
        this.alertService.open({ title: 'Error', message: 'Ocurrió un error al guardar la tarea.', type: 'error' });
      }).finally(() => {
